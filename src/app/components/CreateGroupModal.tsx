...
import { X, Users, Sparkles, ArrowRight, Loader2 } from "lucide-react";
...
export function CreateGroupModal({ isOpen, onClose, onGroupCreated }: CreateGroupModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setCreating(false);
    if (error || !data) {
      setError(error ?? "No se pudo crear el grupo.");
      return;
    }
    const emoji = emojiMap[groupType] || "✈️";
    onGroupCreated(data.id, data.name, data.invite_code, emoji);
    reset();
  };
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="relative p-8">
          ...
          <div className="mb-8">
            ...
            <h2 className="text-3xl font-bold text-foreground mb-2">Crear grupo</h2>
            <p className="text-muted-foreground">Organiza tu próxima aventura con amigos</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={creating}
              className="w-full py-4 bg-gradient-to-r from-primary to-primary/90 text-white rounded-2xl hover:shadow-lg hover:scale-[1.02] transition-all flex items-center justify-center gap-2 mt-8 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              {creating ? (
                <>
