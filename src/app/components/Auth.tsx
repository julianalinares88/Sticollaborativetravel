...
import { Mail, Lock, User, Upload, ArrowRight, ArrowLeft, Loader2 } from "lucide-react";
...
// Traduce los mensajes de error de Supabase (vienen en inglés) al español.
...
export function Auth({ onAuthSuccess, onBack }: AuthProps) {
  ...
  // Opción A: la foto es solo vista previa local; aún no se sube a Supabase Storage.
  ...
  const handleGoogle = async () => {
    ...
    if (error) setError(translateAuthError(error));
    // Si todo va bien, el navegador redirige a Google automáticamente.
  };
      ...
      <div className="hidden lg:block relative overflow-hidden bg-gradient-to-br from-primary/20 to-accent/20">
        ...
        <div className="relative z-10 h-full flex flex-col justify-between p-12 text-white">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-semibold">TripMatch</span>
          </div>
      ...
      <div className="flex items-center justify-center p-6 lg:p-12 bg-white relative">
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {(error || info) && (
              <div
                className={`rounded-2xl text-sm px-4 py-3 ${
                  error
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-gradient-to-r from-primary to-primary/90 text-white rounded-2xl hover:shadow-lg hover:scale-[1.02] transition-all flex items-center justify-center gap-2 group mt-6 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              {loading ? (
                <>
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-3 border border-border rounded-2xl hover:bg-muted transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
          >
